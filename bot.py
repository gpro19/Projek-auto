...
# Konfigurasi
BOT_TOKEN = "8156404642:AAGUomSAOmFXOoj2Ndka1saAA_t0KjC2H9Q" # Ganti token jika perlu
...
# User-Agent header to make requests look more like a browser
HEADERS = {
...
def create_transaction(user_id, product_price):
    """
    Creates a new QRIS transaction via ForestAPI H2H API with retries.
    ...
    try:
        response = http.get(f"{BASE_URL}/api/h2h/deposit/create", params=params, headers=HEADERS, timeout=15)
...
def check_payment_status(transaction_id):
    """
    Checks the status of a ForestAPI transaction with retries.
    ...
    try:
        response = http.get(f"{BASE_URL}/api/h2h/deposit/status", params=params, headers=HEADERS, timeout=15)
